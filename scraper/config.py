"""站點設定：Phase 0 只鎖定客戶指定的三站。

新增站點時只要在 SITES 補一筆，其餘流程（正規化 / 比對 / 報表 / 前端）不用動。
"""

from dataclasses import dataclass, field


@dataclass
class SiteConfig:
    key: str                    # 內部代號
    name: str                   # 顯示名稱
    base_url: str               # 站台根網址
    kind: str                   # "shopserve" | "fc2cart"
    start_urls: list = field(default_factory=list)   # 分類清單的起點
    # 每次 HTTP 請求之間的間隔秒數。FC2 站對機器人較敏感，拉長。
    delay: float = 2.5
    # 單次執行的分類數上限（0 = 不限）。PoC 想快速試跑時可調小。
    max_categories: int = 0
    # 單一分類最多翻幾頁的保險絲，避免分頁探測失誤造成無限迴圈。
    # FC2 每頁只放 21 件，1495 件的分類要翻 72 頁，設太小會默默少抓一半。
    max_pages: int = 120
    # 首頁找到幾個分類就算夠、不再往下鑽一層
    enough_categories: int = 10
    enabled: bool = True


SITES = [
    SiteConfig(
        key="home_slot",
        name="ホームスロット",
        base_url="https://home-slot.net",
        kind="shopserve",
        start_urls=["https://home-slot.net/"],
        delay=2.0,
    ),
    SiteConfig(
        key="a_slot",
        name="A-SLOT",
        base_url="https://www.a-slot.com",
        kind="shopserve",
        start_urls=["https://www.a-slot.com/"],
        delay=2.0,
    ),
    SiteConfig(
        key="initialp",
        name="イニシャルP (FC2)",
        base_url="https://initialp.cart.fc2.com",
        kind="fc2cart",
        start_urls=["https://initialp.cart.fc2.com/"],
        # 這站對抓取頻率最敏感：短時間內重複測試就會從 200 變成 403。
        # 間隔拉到 7 秒，寧可慢也不要被鎖。
        delay=7.0,
        # 側欄第一層就有「中古パチンコ 一覧 / 中古パチスロ 一覧」兩個分類，
        # 加起來 2687 件已經涵蓋全部機台。再往下鑽會多出 100 多個細分類，
        # 商品完全重複，只是白白多花半小時。
        enough_categories=2,
    ),
]

USER_AGENT = (
    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 "
    "(KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36"
)

# 抓到的商品數比上一次掉超過這個比例就發出告警（代表網站可能改版了）
DROP_ALERT_RATIO = 0.5

# 機種模糊比對門檻
MATCH_AUTO = 90     # >= 自動視為同一機種
MATCH_REVIEW = 75   # >= 進人工待確認佇列，< 視為不同機種

# ---------------------------------------------------------------------------
# 「這筆到底是不是一台機器」的判斷
#
# 三站除了實機，也賣配件與服務：コイン不要機、データカウンター、
# 音量加工、不要台回收、10 円玉加工⋯⋯這些跟實機混在同一批分類頁裡。
# 拿它們去比價毫無意義，還會把畫面洗版，所以在入庫前就濾掉。
# ---------------------------------------------------------------------------

# 第一道也是最可靠的一道：整個大分類直接排除。
#
# 兩站的頂層分類本來就分好了 —— home-slot 有「スロット実機 / パチンコ実機」
# 與「スロットオプション / パチンコオプション」，A-SLOT 有「スロット実機」
# 與「オプション」。大分類 ID 就寫在網址 /SHOP/{大分類}/{小分類}/list.html 裡，
# 照著站方自己的分類走，比從商品名去猜準得多。
OPTION_CATEGORY_KEYWORDS = [
    "オプション", "サービス", "グッズ", "回収", "処分",
    "周辺機器", "パーツ", "部品", "アクセサリ",
]

# 「全部商品」這種總覽分類要跳過：內容跟其他分類完全重複，
# 而且它是連配件一起列的，抓了等於把剛濾掉的配件又放回來。
OVERVIEW_CATEGORY_KEYWORDS = ["全件表示", "全商品", "すべての商品", "全ての商品"]

# 中古實機再便宜也不會低於這個價；¥3,000 的「回收服務」一定不是機台
MIN_MACHINE_PRICE = 15000

# 機種名上限。分類過濾接手之後這裡就不必抓太緊，
# 免得誤殺「サミー Lパチスロ 甲鉄城のカバネリ 海門決戦 中古パチスロ実機［スマスロ］」
# 這種完整寫法（已經 40 字）
MAX_NAME_LENGTH = 60

# 命中任一個就排除。選詞刻意保守，只放明確不可能出現在機種名裡的字眼
NON_MACHINE_KEYWORDS = [
    "お客様の声", "レビュー",
    "サービス", "回収", "加工", "取付", "延長保証", "保証書",
    "コイン不要機", "データカウンター", "カウンター",
    "ドル箱", "スタンド", "キャビネット", "テーブル", "椅子", "イス",
    "電源スイッチ", "スイッチ", "リモコン", "アダプター", "変換",
    "送料", "配送料", "代引", "設置費",
    "メダル", "パチンコ玉", "球貸", "補給",
    "クリーナー", "洗浄", "清掃", "工具", "部品", "パーツ",
]
