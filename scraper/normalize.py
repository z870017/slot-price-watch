...
# ホームスロット的柏青哥商品會把「枠」（外框）名稱塞在括號裡：
# 「Pとある魔術の禁書目録2 [枠名：クリスタルエアー とある魔術２]」。
# 這段必須整段刪掉，不能只拆括號留內容 —— 框名常常帶著機種名和數字
# （上例的「とある魔術２」），留下來會污染比對字串和數字指紋，
# 害同一台機在別站的乾淨名稱永遠配不上。1066 筆商品都有這問題。
FRAME_RE = re.compile(r"[【［\[（(〔][^】］\]）)〕]*枠名[^】］\]）)〕]*[】］\]）)〕]")
